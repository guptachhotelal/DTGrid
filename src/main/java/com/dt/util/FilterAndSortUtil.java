...
public class FilterAndSortUtil {
	...
	public static <T> List<T> filter(Collection<T> data, String searchText) {
		Predicate<T> predicate = new FilterPredicate<>(searchText);
		return data.parallelStream().filter(predicate).toList();
	}

	public static <T> List<T> sort(Collection<T> data, String sortColumn, boolean asc) {
		Comparator<T> comparator = new FieldComparator<>(sortColumn, asc);
		return data.parallelStream().sorted(comparator).toList();
	}

	public static <T> Map<Long, List<T>> filterAndSort(Collection<T> data, String searchText, int pgNumber, int pgSize,
